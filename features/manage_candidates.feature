Feature: Manage candidates
  ...
  Scenario: Check errors when only name is filled
    ...
    And I press "Создать"
    Then I should see errors like "Desired position can't be blank" and "Status can't be blank"

  Scenario: Check errors when only status is filled
    Given I have logged in user
    And I am on the new candidate page
    When I fill in "candidate_status" with "Some status"
    And I press "Создать"
    Then I should see errors like "Name can't be blank" and "Desired position can't be blank"

  Scenario: Check errors when only position is filled
    Given I have logged in user
    And I am on the new candidate page
    When I fill in "candidate_desired_position" with "Some position"
    And I press "Создать"
    Then I should see errors like "Name can't be blank" and "Status can't be blank"
