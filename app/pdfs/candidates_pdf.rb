class CandidatesPdf < Prawn::Document
  ...
  def initialize(candidates)
    ...
    font_families.update('OpenSans' => { normal: Rails.root.join('app','assets','fonts','OpenSans-Regular.ttf') })
    font 'OpenSans', size: 10 do
      data = [ %w{ Кандидат Должность Регион Зарплата Ответственный Добавлен Статус Примечание } ]
      candidates.each do |candidate|
        data += [[ candidate.name, candidate.desired_position, candidate.city_of_residence, candidate.salary,
